...
run_nmap_quick() {
    ...
    # In lite mode prefer masscan directly for speed
    if is_lite; then
        log_info "Lite mode enabled – using masscan for quick port discovery"
        if check_tool "masscan"; then
            # Use a reduced port range to limit noise
            require_root || return 1
            local outfile
            outfile=$(generate_target_filename "$target" "masscan_quick" "xml")
            log_command_preview "masscan -p1-1000 --rate=2000 -oX $outfile $target"
            log_audit "SCAN" "masscan_quick" "$target"
            local start_time_mass
            start_time_mass=$(date +%s)
            masscan -p1-1000 --rate=2000 -oX "$outfile" "$target"
            local duration
            duration=$(elapsed_time "$start_time_mass")
            operation_summary "success" "Masscan Quick" "Output: $outfile\nDuration: $duration"
            log_loot "Masscan results: $outfile"
            return
        fi
        log_warning "masscan not available – falling back to nmap"
    fi
...
run_nmap_full() {
    ...
    # In lite mode avoid heavy full scanning and fall back to quick scan
    if is_lite; then
        log_info "Lite mode enabled – using quick scan instead of full scan"
...
run_nmap_udp() {
    ...
    # Skip UDP scanning entirely in lite mode as it is slow and noisy
    if is_lite; then
        log_info "Lite mode enabled – skipping UDP scan"
        return 0
    fi
